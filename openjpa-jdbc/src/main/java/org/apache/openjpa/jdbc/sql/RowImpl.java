...
public class RowImpl implements Row, Cloneable {
	...
	public static String getPartitionTableName(String originalTableName, Integer runid, Long sourceid) {
		...
		// normal insert as no partitions were found
	...
	public static PartitionRecord<Long> getMaxLevelPartitionRecordFor(String tableName) {
		...
		return maxLevelPartitionRecord;
	}

	public int lookupColumnIndexOfPartitionKey(String partitionKey) {
		for (int i = 0; i < this._cols.length; i++) {
			if (this._cols[i].getIdentifier().getName().equals(partitionKey)) {
				return i;
			}
		}
